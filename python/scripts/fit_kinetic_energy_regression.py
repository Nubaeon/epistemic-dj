"""Fits kinetic_energy's audio-feature -> value mapping against DEAM's
human-rated arousal annotations, replacing the hand-picked heuristic
weights that were never validated against anything.

DEAM (Database for Emotional Analysis in Music, Soleymani/Aljanaki/Yang) --
CC BY-NC licensed (David accepted the licensing question, 2026-07-19,
see empirica decision 5e25c172). 1802 royalty-free excerpts/songs with
static (whole-song) arousal/valence ratings on a [1,9] scale from 5+ human
raters each. arousal is the closest established construct to kinetic_energy.

Run from python/: uv run python scripts/fit_kinetic_energy_regression.py
"""
...
OUTPUT_PATH = Path(__file__).parent.parent / "epistemic_dj" / "audio" / "kinetic_energy_model.json"
...
def load_static_annotations() -> dict[int, float]:
    """song_id -> arousal, rescaled from DEAM's [1,9] scale to [0,1]."""
    arousal_by_id: dict[int, float] = {}
    for csv_path in ANNOTATIONS_DIR.glob("static_annotations_averaged_songs_*.csv"):
        with open(csv_path, newline="") as f:
            reader = csv.DictReader(f, skipinitialspace=True)
            for row in reader:
                song_id = int(row["song_id"])
                arousal_1_9 = float(row["arousal_mean"])
                arousal_by_id[song_id] = (arousal_1_9 - 1.0) / 8.0
    return arousal_by_id
...
def main() -> None:
    arousal_by_id = load_static_annotations()
    print(f"Loaded {len(arousal_by_id)} static arousal annotations.")

    with zipfile.ZipFile(AUDIO_ZIP) as zf:
        audio_by_id = find_audio_names(zf)
        print(f"Found {len(audio_by_id)} audio files in archive.")

        available_ids = sorted(set(arousal_by_id) & set(audio_by_id))
        ...
        for i, song_id in enumerate(sample_ids):
            ...
            try:
                ...
                targets.append(arousal_by_id[song_id])
    ...
    output = {
        ...
        "n_test": len(y_test),
        "n_failed": len(failed),
        "source": "DEAM (CC BY-NC), static arousal annotations rescaled [1,9]->[0,1]",
